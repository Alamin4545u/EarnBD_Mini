...
document.addEventListener("visibilitychange", () => {
    // শুধুমাত্র যদি টাস্ক পেন্ডিং থাকে এবং ইউজার ১০ সেকেন্ড পর ফিরে আসে
...
window.handleTask = async (tid, rew, type, link) => {
    ...
    if (type === 'direct_ad') {
        ...
        if(url) { 
            ...
            // ব্যাকগ্রাউন্ডে ইন্টারস্টিশিয়াল লোড
            setTimeout(() => { 
        ...
        if(adFuncs.rewarded && window[adFuncs.rewarded]) {
            window[adFuncs.rewarded]().then(() => { 
                // ভিডিও দেখলে সরাসরি ক্লেইম, টাইমার রিসেট
                claimReward(tid, rew); 
                adStartTime = 0; 
                pendingTask = null; 
            });

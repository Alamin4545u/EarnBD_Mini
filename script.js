...
async function initApp() {
  ...
  try {  
    // প্যারালাল ডাটা ফেচিং (Fast Loading)  
    ...
    // অ্যাড স্ক্রিপ্ট লোড এবং চেক  
    if (appSettings.monetag_interstitial_id) {
      loadScript(appSettings.monetag_interstitial_id, (n) => {
        adFuncs.interstitial = n;
        if (!window[n]) console.warn('Interstitial ad not loaded');
      });
    }  
    if (appSettings.monetag_rewarded_id) {
      loadScript(appSettings.monetag_rewarded_id, (n) => {
        adFuncs.rewarded = n;
        if (!window[n]) console.warn('Rewarded ad not loaded');
      });
    }  
    if (appSettings.monetag_popup_id) {
      loadScript(appSettings.monetag_popup_id, (n) => {
        adFuncs.popup = n;
        if (!window[n]) console.warn('Popup ad not loaded');
      });
    }  
    ...
    if (!userRes.data) {  
      ...
      if (refId) {  
        // ব্যাকগ্রাউন্ডে রেফার বোনাস প্রসেস হবে  
        supabase.rpc('increment_referral', { referrer_id: refId }).then();  
...
async function renderTasks(c) {
  ...
  const { data: logs } = await supabase.from('task_logs').select('task_id').eq('user_id', currentUser.id).gte('created_at', today + 'T00:00:00').lt('created_at', today + 'T23:59:59');  // ফিক্স: date range দিয়ে logs ফেচ
...
window.handleTask = async (tid, rew, type, link) => {
  if (!adFuncs.interstitial && !adFuncs.rewarded && !adFuncs.popup) {
    return Swal.fire('Error', 'Ads not loaded. Try refreshing.', 'error');
  }

  pendingTask = { id: tid, reward: rew };
  adStartTime = Date.now(); // টাইমার শুরু

  if (type === 'direct_ad') {  
    const url = (link && link !== 'null') ? link : appSettings.monetag_direct_link;  
    if (url) {   
      window.open(url, '_blank');   
      // ব্যাকগ্রাউন্ডে ইন্টারস্টিশিয়াল লোড  
      setTimeout(() => {   
    ...
    if (adFuncs.rewarded && window[adFuncs.rewarded]) {  
      window[adFuncs.rewarded]().then(() => {   
        // ভিডিও দেখলে সরাসরি ক্লেইম  
        claimReward(tid, rew);   
        resetPending();   
      }).catch((err) => {
        Swal.fire('Error', 'Ad failed: ' + err.message, 'error');
        resetPending();
      });  
    } else {
      Swal.fire('Error', 'Rewarded ad not available', 'error');
    }  
  } else {  
  ...
  // ফলব্যাক: 12 সেকেন্ড পর কনফার্ম বাটন শো (visibilitychange না কাজ করলে)
  setTimeout(() => {
    if (pendingTask) {
      Swal.fire({
        title: 'Did you complete the task?',
        text: 'Confirm to claim reward.',
        icon: 'question',
  ...
  }, MIN_AD_DURATION + 2000);  // 12 সেকেন্ড পর
};
...
async function claimReward(tid, rew) {
  ...
  if (res && res.success) {  
    ...
    Swal.fire({ icon: 'error', title: 'Oops', text: res?.message || 'Claim failed' });  
  }
}
...
async function processWithdraw() {
  ...
  if (res && res.success) {  
    ...
    btn.disabled = false; // ফেইল হলে বাটন ওপেন  
...
window.copyLink = async () => {
  const copyText = document.getElementById("ref-link").value;
  try {
